...
long long EARLY_STOP = 0, max_train_words;

int XLING_WAIT = 0, waiting_for_xling = 0;
real *xling_delta, XLING_LAMBDA = 0.001;
...
void *TrainModelThread(void *id) {
  ...
  if (!EARLY_STOP)
    for (a = 0; a < NUM_LANG; a++) all_train_words += max_train_words; //train_words[a];
  else {
  ...
  while (1) {
    if (word_count - last_word_count > 10000) {
      ...
      if ((debug_mode > 1)) {
        now=clock();
        printf("%cAlpha: %f  Progress: %.2f%%  Updates (L1: %lld, L2: %lld) Words/sec: %.2fk  ", 13, alpha,
    ...
    // TODO: take num_threads into account below
    if (word_count > max_train_words) break;
    if (feof(fi)) {
      fseek(fi, 0, SEEK_SET);
      continue;
    }
    if (EARLY_STOP) if (word_count > EARLY_STOP) {
...
void TrainModel() {
  ...
  num_threads = NUM_LANG + 1;       // TODO: Change this to support multiple langs and threads per lang
  for (a = 0; a < num_threads - 1; a++) pthread_create(&pt[a], NULL, TrainModelThread, (void *)a);
